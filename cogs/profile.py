...
class Profile(commands.Cog):
    ...
    async def get_social_page(self, ctx, member: discord.Member):
        ...
        if info['twitter']:
            embed.add_field(name=str(self.twitter_emoji) + 'Twitter', value=f'[{info["twitter"] if info["twitter"].startswith("@") else "@" + info["twitter"]}](https://twitter.com/{info["twitter"].replace("@", "")})', inline=True)
        if info['reddit']:
